  ...
  <div class="card shadow-none">
    <div class="card-header">
      <ul class="nav nav-pills">
        <li class="nav-item">
          <a class="nav-link active" href="/console/service/{{ $service->service_id }}/basics">Basic settings</a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="/console/service/{{ $service->service_id }}/line-messaging-api">LINE Messaging API</a>
        </li>
        <li class="nav-item">
    ...
    <h2 class="pb-4">Basic settings</h2>
      ...
      <form id="service-update" action="/console/service/{{ $service->service_id }}/update" method="POST">
        @csrf
        ...
        <hr>
        <div class="form-group row">
          <label for="service_name" class="col-lg-2 col-form-label">Service name</label>
          <div class="col-lg-10">
            <input type="text" class="form-control @error('service_name') is-invalid @enderror" id="service_name" name="service_name" value="{{ $service->service_name }}" aria-describedby="ServiceHelp">
            @error('service_name')
            ...
            <small id="ServiceHelp" class="form-text text-muted">Don't use special characters (4-byte Unicode)</small>
            <small id="ServiceHelp" class="form-text text-muted">Enter no more than 100 characters</small>
            <p class="mt-2">
      ...
      <form id="service-delete" action="/console/service/{{ $service->service_id }}/delete" method="POST">
        @csrf
        <button type="button" class="btn btn-danger btn-flat" onclick="event.preventDefault(); document.getElementById('service-delete').submit();">Delete</button>
      </form>
    </div>
    <!-- /.card-body -->
